...
sample_seg_path = (
    "test_data/sub-verse004/derivatives/seg_roi/sub-verse004_vert-16-seg-vert_msk.nii.gz"
)
...
rotated_ct = rotate_about_image_center(
    ct, rx=0, ry=0, rz=360 - rot_angle, interpolator=get_interpolator("linear")
)
write_image(rotated_ct, out_ct_path, pixeltype=ct.GetPixelID())

seg = read_image(sample_seg_path)
rotated_seg = rotate_about_image_center(
    seg, rx=0, ry=0, rz=360 - rot_angle, interpolator=get_interpolator("nearest")
)
...
generate_perturbed_xray(sample_ct_path, ProjectionType.AP, config, out_xray_ap_path, rot_angle)
...
def save_projections(transforms):
    ...
    if len(ap_img.shape) == 3:
        ap_img = ap_img[0]
    elif len(ap_img.shape) == 4:
        ap_img = ap_img[0, 0]
    lat_img = lat_dict["lat"]
    if len(lat_img.shape) == 3:
        lat_img = lat_img[0]
    elif len(lat_img.shape) == 4:
        lat_img = lat_img[0, :, :, 0]
...
ap_img_path = (
    out_xray_ap_path
) = "2D-3D-Reconstruction-Datasets/verse19/subjectwise/sub-verse004/derivatives/xray_from_ct_augment/sub-verse004_vert-16_angle-30_ap.png"
lat_img_path = (
    out_xray_lat_path
) = "2D-3D-Reconstruction-Datasets/verse19/subjectwise/sub-verse004/derivatives/xray_from_ct_augment/sub-verse004_vert-16_angle-30_lat.png"
seg_img_path = (
    out_seg_path
) = "2D-3D-Reconstruction-Datasets/verse19/subjectwise/sub-verse004/derivatives/seg_roi_augment/sub-verse004_vert-16_angle-30-seg-vert_msk.nii.gz"
