# from https://github.com/graphdeeplearning/benchmarking-gnns/blob/master/utils/cleaner_main.py
...
def cleaner_main(filename):
    ...
    if idx_start_fnc != None:
        idx_end_fnc = idx_start_fnc + next(
            (
                i
                for i, x in enumerate(lines_in[idx_start_fnc + 1 :])
                if x[:4] not in ["\n", "    "]
            ),
            None,
        )
        lines_in = [
            ...
            if i not in range(idx_start_fnc, idx_end_fnc + 1)
        ]
    ...
    idx_start_fnc = next(
        (i for i, x in enumerate(lines_in) if "if notebook_mode==False" in x), None
    )
    if idx_start_fnc != None:
        idx_end_fnc = idx_start_fnc + next(
            (
    ...
    while run:
        idx_start_fnc = next(
            (i for i, x in enumerate(lines_in) if x[:16] == "if notebook_mode"), None
        )
        if idx_start_fnc != None:
            idx_end_fnc = idx_start_fnc + next(
                (
    ...
    idx_start_fnc = next(
        (i for i, x in enumerate(lines_in) if x[:20] == "    if notebook_mode"), None
    )
    if idx_start_fnc != None:
        idx_end_fnc = idx_start_fnc + next(
            (
