...
with open("scripts/script_templates/evaluate_embed_decode_angle_preturbation_template.sh") as f:
    for idx, angle in enumerate(ANGLE_PERTURBATIONS):
        angle_perturbation_csv_path = (
            Path(args.testpaths)
            .with_name(Path(args.testpaths).stem + "_" + str(angle))
            .with_suffix(".csv")
        )
