...
for model_name in MODEL_NAMES:
    ...
    latex_table += latex_table_row_template.format(
        ...
        FHR=df.median(numeric_only=True).FHR,
        FHC=df.median(numeric_only=True).FHC,
        NSA=df.median(numeric_only=True).NSA,
        FNA_x=df.median(numeric_only=True).FNA_x,
        FNA_y=df.median(numeric_only=True).FNA_y,
        FNA_z=df.median(numeric_only=True).FNA_z,
        FDA_x=df.median(numeric_only=True).FDA_x,
        FDA_y=df.median(numeric_only=True).FDA_y,
        FDA_z=df.median(numeric_only=True).FDA_z,
