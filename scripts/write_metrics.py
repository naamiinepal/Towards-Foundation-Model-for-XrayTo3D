...
with open(Path(target_dir) / "metric-log.csv", "w") as f:
    ...
    NSD = lambda gt, pred: compute_surface_overlap_at_tolerance(
        compute_surface_distances(gt.astype(bool), pred.astype(bool), VOXEL_SPACING), NSD_TOLERANCE
    )[0]
    ...
    for gt_p, pred_p in zip(gt_paths, pred_paths):
        ...
        dsc = DSC(torch.from_numpy(gt), torch.from_numpy(pred))
        dsc = to_numpy(dsc).flatten().tolist()[0]

        asd = ASD(torch.from_numpy(gt), torch.from_numpy(pred))
        asd = to_numpy(asd).flatten().tolist()[0]

        hd95 = HD95(torch.from_numpy(gt), torch.from_numpy(pred))
        hd95 = to_numpy(hd95).flatten().tolist()[0]

        nsd = NSD(gt[0, 0], pred[0, 0])
        # print(f'{subject_id} shape {gt.shape} DSC {dsc} ASD {asd} HD95 {hd95} NSD {nsd}' )
        filestream_writer.writerow(
            [subject_id, f"{dsc:.2f}", f"{asd:.2f}", f"{hd95:.2f}", f"{nsd:.2f}"]
        )
