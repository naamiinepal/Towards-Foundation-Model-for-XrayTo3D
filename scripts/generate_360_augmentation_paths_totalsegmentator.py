...
def get_individual_fullpaths_aug(
    ...
    for augment_angle in range(0, 360, 5):
        out_seg_aug_path = generate_augment_path(
            "seg_roi_augment",
            "seg_roi_aug",
            subject_id,
            augment_angle,
            output_path_template=OUT_PATH_TEMPLATE,
            config=config,
        )

        out_xray_ap_path = generate_augment_path(
            "xray_from_ct_augment",
            "xray_aug_ap",
            subject_id,
            augment_angle,
            OUT_PATH_TEMPLATE,
        ...
        out_xray_lat_path = generate_augment_path(
            "xray_from_ct_augment",
            "xray_aug_lat",
            subject_id,
            augment_angle,
...
def get_fullpaths(
    ...
    if config["dataset"] == "verse":
        for subject, subject_dir in subject_list:
            if aug_based:
                _ap, _lat, _seg = get_individual_fullpaths_aug(subject_dir, config)
            else:
                _ap, _lat, _seg = get_individual_fullpaths(
                    subject_dir, config, ap_pattern, lat_pattern, seg_pattern, False
                )
            ap.extend(_ap)
            lat.extend(_lat)
            seg.extend(_seg)
        ...
        for subject in subject_list:
            if aug_based:
                _ap, _lat, _seg = get_individual_fullpaths_aug(subject, config)
            else:
                _ap, _lat, _seg = get_individual_fullpaths(
                    subject, config, ap_pattern, lat_pattern, seg_pattern, False
                )
            ap.extend(_ap)
            lat.extend(_lat)
            seg.extend(_seg)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", default=None, required=None)
    ...
    save_path = Path(config_path).parent / args.save_dir / "filename_placeholder.txt"
    # create intermediate save directories if required
    save_path.parent.mkdir(parents=True, exist_ok=True)
    for df, suffix in zip(
        [df_train, df_train_noaug, df_val, df_test, df_train_val],
        ["train", "train_noaug", "val", "test", "train+val"],
    ):
        df.to_csv(
            Path(save_path).with_name(Path(config_path).stem + "_" + suffix).with_suffix(".csv")
        )
